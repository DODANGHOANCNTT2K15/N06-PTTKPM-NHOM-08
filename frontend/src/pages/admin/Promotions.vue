      ...
      try {
        ...
        if (response.status === 200 && response.data.err === 0) {
          ...
          Swal.fire({
            icon: "error",
            title: "Lỗi",
            text: response.data.msg || "Không thể tải danh sách khuyến mại!",
          });
        }
      } catch (error) {
        ...
        Swal.fire({
          icon: "error",
          title: "Lỗi",
          text: "Có lỗi xảy ra khi tải danh sách khuyến mại!",
        });
      }
    };
    ...
    const savePromotion = async () => {
      ...
      try {
        ...
        if (editingPromotion.value) {
          ...
          response = await apiAddDiscount(payload);
        }
        ...
        if (response.data.err === 0) {
          ...
          Swal.fire({
            icon: "error",
            title: "Thất bại",
            text: response.data.msg || "Không thể lưu khuyến mại!",
          });
        }
      } catch (error) {
        ...
        Swal.fire({
          icon: "error",
          title: "Lỗi",
          text: "Có lỗi xảy ra khi lưu khuyến mại!",
        });
      }
    };
    ...
    const deletePromotion = async (id) => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            const response = await apiDeleteDiscount({ discount_id: id });
            if (response.data.err === 0) {
              await fetchPromotions();
              if (
                filteredAndSortedPromotions.value.length === 0 &&
                currentPage.value > 1
              ) {
                currentPage.value--;
              }
              ...
              Swal.fire({
                icon: "error",
                title: "Thất bại",
                text: response.data.msg || "Không thể xóa khuyến mại!",
              });
            }
          } catch (error) {
            ...
            Swal.fire({
              icon: "error",
              title: "Lỗi",
              text: "Có lỗi xảy ra khi xóa khuyến mại!",
            });
          }
        }
      });
    ...
    const filteredAndSortedPromotions = computed(() => {
      ...
      if (sortKey.value) {
        result.sort((a, b) => {
          const valueA =
            a[
              sortKey.value === "discount" ? "percent_discount" : sortKey.value
            ];
          const valueB =
            b[
              sortKey.value === "discount" ? "percent_discount" : sortKey.value
